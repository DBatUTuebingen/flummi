...
def make_lexer[T: Tokens](types: type[T], skip: set[T] = set()) -> Lexer[T]:
  ...
  def lex(code: str) -> Iterator[Token]:
    token_regex = r'|'.join(fr'(?P<{type.name}>{type.value})' for type in types)
...
class Parser[T: Tokens]:
  ...
  def expectv(self, type: T, msg: str | None = None) -> str:  # type: ignore
    ...
    if (result := self.multi_expectv(type, msg=msg)) is not None:
      _, value = result
      return value
  ...
  def multi_expectv(self, *types: T, msg: str | None = None) -> tuple[T, str]:
    ...
    :rtype: tuple[T, str]
    """
    msg = msg or f"Expected one of ({', '.join(type.name for type in types)}) but found {self.current.type.name}."
    if (result := self.multi_matchv(*types)) is not None:
      return result
    else:
